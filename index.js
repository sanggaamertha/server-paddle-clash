...
function startBotMatchTimer(socket) {
    ...
    console.log(`Pemain ${socket.id} menunggu. Timer bot diatur untuk ${Math.round(delay/1000)} detik.`);
...
function matchWithBot(playerSocket) {
    ...
    games[roomId] = {
        ...
        players: {
            [playerSocket.id]: { isPlayer1: true, paddleX: 200, paddleY: 700, paddleVX: 0, paddleVY: 0 },
            [botId]: { isPlayer1: false, paddleX: 200, paddleY: 100, paddleVX: 0, paddleVY: 0, isBot: true },
        },
        ...
        if (game.puck.isFrozen) {
            ...
            if (!game.puck.countdownStarted) {
                ...
                const countdownInterval = setInterval(() => {
                    if (games[roomId]) { // Pastikan game masih ada
                        ...
                        if (count > 0) {
                            ...
                            const scorerIsPlayer1 = game.lastScorer ? games[roomId].players[game.lastScorer].isPlayer1 : Math.random() > 0.5;
...
function updateBotPaddle(game, dt) {
    ...
    if(puck.vy < 0) { // Puck bergerak ke arah bot
    ...
    targetX = Math.max(PADDLE_RADIUS, Math.min(ARENA_WIDTH - PADDLE_RADIUS, targetX));
...
function handleGoalScoring(game) {
    ...
    const goalTopY = 10, goalBottomY = ARENA_HEIGHT - 10, goalLeftX = ARENA_WIDTH * 0.25, goalRightX = ARENA_WIDTH * 0.75;
    let scorerId = null;
    if (puck.y < goalTopY && puck.x > goalLeftX && puck.x < goalRightX) scorerId = Object.keys(players).find(id => players[id].isPlayer1);
    else if (puck.y > goalBottomY && puck.x > goalLeftX && puck.x < goalRightX) scorerId = Object.keys(players).find(id => !players[id].isPlayer1);
...
server.listen(PORT, () => console.log(`Dedicated server berjalan di port ${PORT}`));